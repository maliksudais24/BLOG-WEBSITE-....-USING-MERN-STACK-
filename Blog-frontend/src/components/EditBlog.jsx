...
export default function EditBlog() {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        setFormData({
          ...
          category_id: blogData.category._id,
        });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      blogFormData.append('category_id', formData.category_id);
      if (image) {
  ...
  // Don't render if not logged in or no blog data
  if (!user || !blog) {
    return null;
  }
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            <label className="block text-gray-300 mb-2 font-medium">
              Cover Image <span className="text-gray-500">(Optional - Leave empty to keep current image)</span>
            </label>
